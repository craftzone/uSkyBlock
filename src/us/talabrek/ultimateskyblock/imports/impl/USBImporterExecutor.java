...
public class USBImporterExecutor {
    ...
    private void doImport(CommandSender sender, USBImporter importer) {
        ...
        uSkyBlock.log(Level.INFO, "Importing " + files.length + " players in chunks of " + chunkSize);
        if (files.length > 0) {
            ...
            public void run() {
                ...
                for (int i = offset; i < files.length && i < offset+chunkSize; i++) {
                    ...
                    if (importer.importFile(plugin, playerFile)) {
                        count++;
                        uSkyBlock.log(Level.FINE, "Successfully imported player-file " + playerFile);
                    } else {
                        failed++;
                        uSkyBlock.log(Level.WARNING, "Could not import player-file " + playerFile);
                    }
                }
                countSuccess += count;
                countFailed += failed;
