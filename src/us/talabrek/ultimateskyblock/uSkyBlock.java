...
public class uSkyBlock extends JavaPlugin implements uSkyBlockAPI {
    ...
    public void onEnable() {
        ...
        reloadConfigs();
        Bukkit.getMessenger().registerOutgoingPluginChannel(uSkyBlock.getInstance(), "BungeeCord");
    ...
    public boolean warpTeleport(final Player player, final PlayerInfo pi) {
        ...
        if (warpSweetWarp == null) {
            player.sendMessage(tr("\u00a74Impossible de vous téléporter à l'île de ce joueur!"));
            return true;
        }
    ...
    public boolean homeSet(final Player player) {
        if (!player.getWorld().getName().equalsIgnoreCase(getSkyBlockWorld().getName())) {
            player.sendMessage(tr("\u00a74Vous devez être sur votre île pour mettre votre home!"));
            return true;
        }
        if (this.playerIsOnIsland(player)) {
            ...
            player.sendMessage(tr("\u00a7aVotre skyblock home a été mis à votre emplacement actuel."));
            return true;
        }
        player.sendMessage(tr("\u00a74Vous devez être sur votre île pour mettre votre home!"));
        return true;
...
//TO DO: test the plugin on the server with the new biome denomination.
    ...
    public static String stripFormatting(String format) {
        ...
        format= format.replaceAll("§l\u27A3 ", "");
        format= format.replaceAll("(\u00a7|&)[0-9a-fklmor]", "");
