...
public class WorldGuardHandler {
    ...
    public static boolean protectIsland(uSkyBlock plugin, CommandSender sender, IslandInfo islandConfig) {
        try {
            ...
            if (islandConfig != null && noOrOldRegion(regionManager, regionName, islandConfig)) {
                ...
                uSkyBlock.log(Level.INFO, "New protected region created for " + islandConfig.getLeader() + "'s Island by " + sender.getName());
                save(regionManager);
            ...
            uSkyBlock.log(Level.SEVERE, "ERROR: Failed to protect " + name + "'s Island (" + sender.getName() + ")", ex);
        }
        return false;
    }
    ...
    private static ProtectedCuboidRegion setRegionFlags(CommandSender sender, IslandInfo islandConfig) throws InvalidFlagFormat {
        ...
        region.setFlag(DefaultFlag.GREET_MESSAGE, DefaultFlag.GREET_MESSAGE.parseInput(getWorldGuard(), sender, "§d\u2713 Vous entrez dans une zone protégée de §b" + islandConfig.getLeader()));
        region.setFlag(DefaultFlag.FAREWELL_MESSAGE, DefaultFlag.FAREWELL_MESSAGE.parseInput(getWorldGuard(), sender, "§d\u2717 Vous quittez une zone protégée de §b" + islandConfig.getLeader() ));
    ...
    public static void islandLock(final CommandSender sender, final String islandName) {
        try {
            ...
            if (regionManager.hasRegion(islandName + "island")) {
                ...
                region.setFlag(DefaultFlag.ENTRY, StateFlag.State.DENY);
                sender.sendMessage(tr("§eVotre île est maintenant verrouillé. Seule les membres de votre groupe peuvent entrer."));
                save(regionManager);
            } else {
                sender.sendMessage(tr("§4Vous devez être le chef de l'île pour pouvoir verrouiller votre île!"));
            }
        } catch (Exception ex) {
            uSkyBlock.getInstance();
			uSkyBlock.log(Level.SEVERE, "ERROR: Failed to lock " + islandName + "'s Island (" + sender.getName() + ")", ex);
        }
    }
    ...
    public static void islandUnlock(final CommandSender sender, final String islandName) {
        try {
            ...
            if (regionManager.hasRegion(islandName + "island")) {
                ProtectedRegion region = regionManager.getRegion(islandName + "island");
                region.setFlag(DefaultFlag.ENTRY, StateFlag.State.ALLOW);
                sender.sendMessage(tr("§eVotre île est déverrouillé et tout le monde peut entrer, mais que vous et les membres de votre groupe peuvent construire."));
                save(regionManager);
            } else {
                sender.sendMessage(tr("§4Vous devez être le chef du groupe pour pouvoir déverrouiller votre île!"));
            }
        } catch (Exception ex) {
            uSkyBlock.getInstance();
			uSkyBlock.log(Level.SEVERE, "ERROR: Failed to unlock " + islandName + "'s Island (" + sender.getName() + ")", ex);
        }
    }
    ...
    public static void removePlayerFromRegion(final String islandName, final String player) {
        ...
        try {
            ...
            uSkyBlock.getInstance();
			uSkyBlock.log(Level.WARNING, "Error saving island region after removal of " + player);
        }
    }
