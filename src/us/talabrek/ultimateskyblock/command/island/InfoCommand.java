...
public class InfoCommand extends RequireIslandCommand {
    public InfoCommand(uSkyBlock plugin) {
        super(plugin, "info", "usb.island.info", "?island", "Verifier les info de votre île ou des autres îles.");
    }
    ...
    protected boolean doExecute(String alias, Player player, PlayerInfo pi, IslandInfo island, Map<String, Object> data, String... args) {
        ...
        if (args.length == 0) {
            ...
            player.sendMessage(tr("\u00a7eVous pouvez utiliser à nouveau cette commande dans {0,number,##.#} secondes.", plugin.getInfoCooldownTime(player) / 1000L));
            return true;
    ...
    public boolean getIslandInfo(final Player player, final String islandPlayer, final String cmd) {
        ...
        if (info == null || !info.getHasIsland() && !plugin.getIslandInfo(info).isParty()) {
            player.sendMessage(tr("\u00a74Ce joueur n'est pas valable ou n'a pas une île!"));
            return false;
        }
        ...
        final Runnable showInfo = new Runnable() {
            ...
            public void run() {
                if (player.isOnline()) {
                    player.sendMessage(tr("\u00a7eInformation sur l'île de " + islandPlayer + ":"));
                    if (cmd.equalsIgnoreCase("info") && shared[0] != null) {
                        ...
                        player.sendMessage(String.format("\u00a7aLe level de l'île est %5.2f", shared[0].getScore()));
                    }
                }
            }
        };
