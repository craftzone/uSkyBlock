...
public class LevelCommand extends RequireIslandCommand {
    public LevelCommand(uSkyBlock plugin) {
        super(plugin, "level", "usb.island.level", "?island", "Voir le niveau de votre île ou des autres îles.");
    }
    ...
    protected boolean doExecute(String alias, Player player, PlayerInfo pi, IslandInfo island, Map<String, Object> data, String... args) {
        ...
        if (args.length == 0) {
            if (!plugin.playerIsOnIsland(player)) {
                player.sendMessage(tr("\u00a7eVous devez être sur votre île pour utiliser cette commande."));
                return true;
            }
            if (!plugin.onInfoCooldown(player) || Settings.general_cooldownInfo == 0) {
                ...
                if (!island.isParty() && !pi.getHasIsland()) {
                    player.sendMessage(tr("\u00a74Vous n'avez pas une île!"));
                } else {
                    getIslandLevel(player, player.getName(), alias);
                }
                return true;
            }
            player.sendMessage(tr("\u00a7eVous pouvez utiliser à nouveau cette commande dans {0} secondes.", plugin.getInfoCooldownTime(player) / 1000L));
            return true;
    ...
    public boolean getIslandLevel(final Player player, final String islandPlayer, final String cmd) {
        ...
        if (info == null || !info.getHasIsland() && !plugin.getIslandInfo(info).isParty()) {
            player.sendMessage(tr("\u00a74Ce joueur n'est pas valide ou n'a pas une île!"));
            return false;
        }
        ...
        final Runnable showInfo = new Runnable() {
            ...
            public void run() {
                if (player.isOnline()) {
                    player.sendMessage(tr("\u00a7eInformations sur l'île de " + islandPlayer + ":"));
                    if (cmd.equalsIgnoreCase("level")) {
                        player.sendMessage(String.format("\u00a7aLe niveau de l'île est de %5.2f", plugin.getIslandInfo(playerInfo).getLevel()));
                    }
                }
            }
        };
